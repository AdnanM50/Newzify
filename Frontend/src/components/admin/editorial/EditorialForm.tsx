...
const CATEGORY_OPTIONS = [
  ...
  { value: "geopolitical-insights", label: "Geopolitical Insights" },
];

const EditorialForm = ({ initialData, onSuccess, onCancel }: EditorialFormProps) => {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setValue("category", initialData.category || "editorial");
      setValue("status", initialData.status || "draft");
      setValue("is_editors_pick", initialData.is_editors_pick || false);
  ...
  const { mutateAsync: create } = useAction(createEditorial, {
    successMessage: "Editorial created successfully",
    invalidateKeys: ["editorials"],
  });

  const { mutateAsync: update } = useAction(updateEditorial, {
    successMessage: "Editorial updated successfully",
    invalidateKeys: ["editorials"],
  });

  const onSubmit = async (data: TEditorial) => {
    try {
  ...
  return (
    <div className="w-full mx-auto bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      <div className="p-6 border-b border-gray-100">
        <h2 className="text-xl font-semibold text-gray-800">{initialData ? "Edit Editorial" : "Create New Editorial"}</h2>
      </div>
      <form onSubmit={handleSubmit(onSubmit)} className="p-8 space-y-6 ">
          ...
          <div className="grid grid-cols-1 lg:col-span-2 space-y-6">
            ...
            <div className="space-y-2">
              ...
              <input
                {...register("subtitle")}
                className="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all"
                placeholder="Optional subtitle (e.g., 'Established procedures must be followed...')"
              />
            </div>
            <div className="space-y-2">
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              ...
              <Controller
                name="category"
                control={control}
                defaultValue={initialData?.category || "editorial"}
                render={({ field }) => (
                  <Select
            ...
            <div className="p-4 bg-gray-50 rounded-lg border border-gray-100 flex items-center justify-between">
              <div>
                <label className="block text-sm font-medium text-gray-900">Editor's Pick</label>
                <p className="text-xs text-gray-500 mt-0.5">Show in the sidebar of the editorials page</p>
              </div>
              <Controller
        ...
        <div className="pt-4 flex justify-end gap-3 border-t border-gray-100">
          ...
          >
            ...
            {uploadingImage ? 'Uploading Image...' : (initialData ? 'Update Editorial' : 'Publish Editorial')}
          </button>
        </div>
      </form>
    </div>
